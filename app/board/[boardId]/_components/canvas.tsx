...
import { connectionIdColor, pointerEventToCanvas } from "@/lib/utils";
...
export const Canvas = ({ boardId }: Props) => {
  ...
  const layerIdsToColorSelection = useMemo(() => {
    const layerIdsToColorSection: Record<string, string> = {};

    for (const user of selections) {
      const [connectionId, selection] = user;

      for (const layerId of selection) {
        layerIdsToColorSection[layerId] = connectionIdColor(connectionId);
      }
    }

    return layerIdsToColorSection;
  }, [selections]);
